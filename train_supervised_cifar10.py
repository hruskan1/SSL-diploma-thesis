...
if __name__ == '__main__':
    ...
    parser.add_argument('--WRN_dropout', default = 0.3, type=float, help='dropout rate for resnet')
    parser.add_argument('-BS','--batch_size',default=128,type = int,help='mini batch size')
    ...
    parser.add_argument('-nt','--n_total',default=50000,type=int,help="number of images available (maximum is what the dataset contains)")
    ...
    num_classes = 10 
    _dataset = tv.datasets.CIFAR10(args.dataset_path, train = True, download = True)
        
    if args.n_total < len(_dataset):
        train_dataloader, _, _,test_dataloader = \
                        datasets.get_CIFAR10(root=args.dataset_path,
                                            n_labeled = args.n_total,
                                            n_val = 0,
    ...
    if args.learning_rate is None:
        losses, lrs = utils.lr_find(model,
                ...
                args = args,
                transform = None,
                min_lr = 1e-7, max_lr = 1e-2, steps = 50,
